...
/*   Updated: 2024/04/23 01:01:52 by sandre-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (valid_map_input(&data))
	{
		ft_printf("Error\n%s", "Invalid map");
		if (data.it.start > 1)
			ft_printf(" - More than one starting point in yout map");
		if (data.it.exit > 1)
			ft_printf(" - More than one exit point in yout map");
		close(fd);
		if (data.map)
			free_map(data.map);
		return (1);
...
int	render_win(t_mlx_data *data)
{
	...
	while (data->map[height])
		height++;
	width = strlen(data->map[height - 1]);
	height *= PIXELS;
	width *= PIXELS;
	if (!data->mlx)
		return (1);
